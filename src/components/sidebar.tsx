"use client";
 ...
 export function Sidebar() {
   ...
   return (
     <UISidebar
       collapsible="icon"
       className="border-r border-slate-100 bg-white shadow-xl shadow-slate-200/50"
     ...
     >
       <SidebarHeader className="py-8 px-6 group-data-[collapsible=icon]:px-0 group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:items-center group-data-[collapsible=icon]:justify-center">
         <Link href="/" className="flex items-center gap-4 w-full group-data-[collapsible=icon]:justify-center">
           ...
           <div className="flex flex-col group-data-[collapsible=icon]:hidden animate-in fade-in slide-in-from-left-4 duration-500">
             ...
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Recruitment OS</p>
           </div>
         </Link>
       </SidebarHeader>
       ...
       <SidebarContent className="px-3 py-4 group-data-[collapsible=icon]:px-2">
         <SidebarGroup>
           <SidebarGroupContent>
             {loadingPerms ? (
               <div className="flex flex-col items-center justify-center py-12 gap-3 text-slate-400 group-data-[collapsible=icon]:hidden animate-pulse">
                   ...
                   return (
                     <SidebarMenuItem key={index}>
                       <SidebarMenuButton
                         ...
                         tooltip={{
                           children: item.name,
                           className: "bg-slate-900 text-white border-none font-black text-[10px] uppercase tracking-widest px-3 py-1.5 shadow-xl",
                         }}
                         className={cn(
                           ...
                           <div className="flex items-center justify-between w-full group-data-[collapsible=icon]:justify-center">
                             <div className="flex items-center gap-3">
                               <div className={cn(
                                 ...
                                 active ? "bg-brand text-white shadow-brand/20 shadow-md" : "bg-slate-50 text-slate-400 group-hover:bg-brand/10 group-hover:text-brand"
                               )}>
         ...
         <div className="mt-8 px-4 group-data-[collapsible=icon]:hidden animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-500">
           <div className="bg-slate-900 rounded-[2rem] p-6 relative overflow-hidden group/pro shadow-2xl shadow-slate-400/20">
             ...
             <div className="relative z-10 flex flex-col gap-4">
               ...
               <div className="space-y-1">
                 ...
                 <p className="text-[10px] font-bold text-slate-400 leading-relaxed uppercase tracking-widest">Unlock AI Matching</p>
               </div>
               <button className="w-full py-2 bg-white text-slate-900 text-[11px] font-black rounded-xl hover:bg-brand hover:text-white transition-all active:scale-95 shadow-xl">
                 Get Started
               </button>
             </div>
           </div>
       ...
       <SidebarFooter className="p-4 mt-auto group-data-[collapsible=icon]:p-2">
         <div className="bg-slate-50 rounded-[2rem] p-3 group-data-[collapsible=icon]:p-0 group-data-[collapsible=icon]:bg-transparent transition-all border border-slate-100/50">
           <div className="flex items-center gap-3 group-data-[collapsible=icon]:justify-center">
             <Avatar className="h-10 w-10 border-2 border-white shadow-sm ring-2 ring-slate-100 shrink-0">
             ...
             <div className="flex flex-col group-data-[collapsible=icon]:hidden min-w-0">
               <span className="text-xs font-black text-slate-900 truncate tracking-tight">{user?.name}</span>
               <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest truncate">{user?.role}</span>
             </div>
             <button 
               onClick={logout}
               className="ml-auto p-2 rounded-xl text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all group-data-[collapsible=icon]:hidden"
             >
