  ...
  return (
    <div className="flex h-full lg:h-[620px] min-h-[500px]">
      ...
      <div className="hidden md:flex flex-col w-52 bg-muted border-r border-border p-8 shrink-0">
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-bold text-foreground text-lg tracking-tight">CliqHire</span>
          </div>
          <p className="text-[10px] text-muted-foreground font-black tracking-widest uppercase">Talent Acquisition</p>
        </div>
          ...
          {TABS.map((tab, index) => (
            <div key={tab} className="flex items-start gap-4">
              ...
              <div className="flex flex-col">
                <span className={`text-sm font-bold ${currentTab === index ? "text-foreground" : "text-muted-foreground"}`}>
                  {tab}
                </span>
                <span className="text-[10px] text-muted-foreground font-bold uppercase">Section {index + 1}</span>
              </div>
            </div>
          ))}
        </div>

        <div className="mt-auto bg-card/60 p-4 rounded-xl border border-border shadow-sm">
      ...
      <div className="flex-1 flex flex-col bg-card overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8 pt-4 min-h-0">
          <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-2">
                    <Label className="text-sm font-bold text-foreground">Full Name <span className="text-primary">*</span></Label>
                    <div className="relative group">
                      <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground group-focus-within:text-primary transition-colors" />
                      <Input name="name" value={form.name} onChange={handleChange} placeholder="John Doe" className="pl-10 h-11 border-border focus:border-primary font-bold" />
                    </div>
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-bold text-foreground">Phone Number <span className="text-primary">*</span></Label>
                  <PhoneInput
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-bold text-foreground">Current Location</Label>
                  <LocationSuggestion
                    value={form.location}
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={form.gender} onValueChange={v => handleSelectChange('gender', v)}>
                      <SelectTrigger className="h-11 border-border font-bold">
                ...
                <div className="space-y-2">
                  ...
                  <Popover open={dobOpen} onOpenChange={setDobOpen} modal={true}>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="w-full h-11 justify-start text-left font-bold border-border px-4">
                        <CalendarIcon className="mr-3 h-4 w-4 text-muted-foreground" />
                        {form.dateOfBirth ? format(form.dateOfBirth, "PPP") : <span className="text-muted-foreground">Select Date</span>}
                      </Button>
                    </PopoverTrigger>
            ...
            {currentTab === 1 && (
              <div className="space-y-6">
                <div className="space-y-2">
                  <Label className="text-sm font-bold text-foreground">Current Job Title</Label>
                  <div className="relative group">
                    ...
                    <Input name="currentJobTitle" value={form.currentJobTitle} onChange={handleChange} placeholder="e.g. Senior Project Manager" className="pl-10 h-11 border-border font-bold" />
                  </div>
                </div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-2 flex flex-col">
                    <Label className="text-sm font-bold text-foreground mb-1">Current Country</Label>
                    <CountrySelect value={form.country} onChange={v => handleSelectChange('country', v)} type="country" placeholder="Lives in..." />
                  </div>
                   <div className="space-y-2 flex flex-col">
                    <Label className="text-sm font-bold text-foreground mb-1">Nationality</Label>
                ...
                <div className="grid grid-cols-2 gap-6 p-6 bg-muted border border-border rounded-2xl">
                  <div className="space-y-2 flex flex-col">
                    <Label className="text-sm font-bold text-foreground mb-1">Continent</Label>
                    <CountrySelect value={form.continent} onChange={v => handleSelectChange('continent', v)} type="continent" placeholder="Select..." />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-sm font-bold text-foreground">Willing to Relocate?</Label>
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-bold text-foreground">Candidate Summary</Label>
                  <div className="relative">
                    <FileText className="absolute right-4 top-4 w-4 h-4 text-muted-foreground" />
                    <textarea name="description" value={form.description} onChange={handleChange} rows={4} className="w-full p-4 border border-border rounded-xl font-medium text-sm focus:border-primary outline-none transition-all custom-scrollbar" placeholder="Briefly describe the candidate's main strengths..." />
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-8 h-full flex flex-col justify-center py-10">
                ...
                <div 
                  ...
                  className={cn(
                    ...
                    form.cv ? "border-green-400 bg-green-50" : "border-border hover:border-primary hover:bg-primary/5 shadow-inner"
                  )}
                >
                  ...
                  {form.cv ? (
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-center -space-x-2">
                        <div className="w-12 h-12 rounded-full bg-card border border-border flex items-center justify-center shadow-sm group-hover:-translate-y-1 transition-transform"><FileText className="w-5 h-5 text-muted-foreground" /></div>
                        <div className="w-12 h-12 rounded-full bg-card border border-border flex items-center justify-center shadow-sm z-10 group-hover:-translate-y-2 transition-transform duration-300"><FileText className="w-5 h-5 text-primary" /></div>
                        <div className="w-12 h-12 rounded-full bg-card border border-border flex items-center justify-center shadow-sm group-hover:-translate-y-1 transition-transform"><FileText className="w-5 h-5 text-muted-foreground" /></div>
                      </div>
                      <div className="space-y-1">
                        <p className="text-xl font-black text-foreground">Drop resume here</p>
                        <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest">or click to browse library</p>
                      </div>
                      <div className="bg-card px-4 py-2 rounded-full inline-block text-[10px] font-black text-muted-foreground border border-border shadow-sm uppercase tracking-tighter">
        ...
        <div className="p-8 bg-muted border-t flex flex-row items-center mt-auto shrink-0 transition-all">
          <div className="flex justify-between w-full h-11 items-center">
            <div className="flex gap-3">
              <Button variant="ghost" onClick={isSubmitting ? undefined : goBack} className="text-muted-foreground font-bold hover:bg-muted">
                Cancel
              </Button>
              {currentTab > 0 && (
                <Button variant="outline" onClick={handlePrevious} className="border-border font-bold bg-card px-5 shadow-sm">
