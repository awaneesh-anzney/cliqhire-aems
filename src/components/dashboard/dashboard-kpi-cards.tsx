         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
             {kpis.map((kpi, idx) => (
                 <div 
                     key={idx}
                     className={cn(
                         "group relative p-5 rounded-[2rem] bg-card border border-border shadow-sm transition-all duration-500 flex flex-col justify-between overflow-hidden",
                         "hover:shadow-xl hover:shadow-black/10 hover:-translate-y-1 hover:border-brand/10",
                         "animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-both",
                         kpi.delay
                     )}
                 >
                     {/* Background Glow Effect */}
                     <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-muted rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                     ...
                     <div className="relative z-10 flex justify-between items-start mb-6">
                         ...
                         <div className="p-1 rounded-full bg-muted text-muted-foreground opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0">
                             <TrendingUp className="w-3.5 h-3.5" />
                         </div>
                     </div>
                     
                     <div className="relative z-10">
                         <p className="text-muted-foreground text-[10px] font-black uppercase tracking-widest mb-1.5 transition-colors group-hover:text-muted-foreground">
                             {kpi.label}
                         </p>
                         <h3 className="text-3xl font-black text-foreground tracking-tight transition-all duration-300 group-hover:scale-[1.02] origin-left">
                             {isLoading ? (
                                 <span className="flex h-8 w-16 bg-muted animate-pulse rounded-md" />
                             ) : (
                                 kpi.value
                             )}
                         </h3>
                         <div className="mt-2 flex items-center gap-1.5">
                             ...
                             <p className="text-[10px] font-bold text-muted-foreground group-hover:text-foreground transition-colors">
