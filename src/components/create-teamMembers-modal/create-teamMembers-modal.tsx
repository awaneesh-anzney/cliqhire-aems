    ...
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) handleClose(); }}>
      <DialogContent className="max-w-4xl p-0 overflow-hidden border-none shadow-2xl rounded-2xl">
        <div className="flex h-full lg:h-[550px] min-h-[400px]">
          {/* Sidebar */}
          <div className="hidden md:flex flex-col w-56 bg-muted border-r border-border p-8 shrink-0">
             <div className="mb-10">
              ...
              <p className="text-[10px] text-muted-foreground font-black tracking-widest uppercase">Team Management</p>
            </div>
              ...
              {TABS.map((tab, index) => (
                <div key={tab} className="flex items-start gap-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-muted-foreground font-bold uppercase">Step {index + 1}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex-1 flex flex-col bg-card overflow-hidden">
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                         <div className="space-y-2">
                          <Label className="text-sm font-bold text-foreground">First Name <span className="text-primary">*</span></Label>
                          <Input 
                            value={formData.firstName} 
                            onChange={(e) => handleInputChange("firstName", e.target.value)} 
                            placeholder="John" 
                            className={cn("h-11 border-border font-bold", errors.firstName && "border-red-500")}
                          />
                      ...
                      <div className="space-y-2">
                        <Label className="text-sm font-bold text-foreground">Work Email <span className="text-primary">*</span></Label>
                        <div className="relative group">
                          ...
                          <Input 
                            ...
                            className={cn("pl-10 h-11 border-border font-bold", errors.email && "border-red-500")}
                          />
                        </div>
                    ...
                    <div className="space-y-8">
                       <div className="space-y-4">
                        ...
                        <Select value={formData.roleId || ""} onValueChange={handleRoleSelect}>
                          <SelectTrigger className={cn("h-12 border-border font-bold bg-muted", errors.teamRole && "border-red-500")}>
                          ...
                          <SelectContent className="z-[100]">
                            {roles.map((role) => (
                              <SelectItem key={role._id || role.id} value={(role._id || role.id) as string} className="py-3 font-bold">
                                ...
                                {role.isSystem && <span className="ml-2 text-[10px] font-black text-muted-foreground bg-muted px-1.5 py-0.5 rounded">SYSTEM</span>}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      ...
                      <div className="space-y-4 pt-4 border-t border-border">
                        ...
                        <div className="relative group">
                          <Input 
                            ...
                            placeholder="••••••••" 
                            className={cn("h-12 border-border font-bold pr-12", errors.password && "border-red-500")}
                          />
                          <button
                            type="button"
                        ...
                        <p className="text-[11px] text-muted-foreground font-semibold px-1">Tip: Use at least 8 characters with numbers and symbols.</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <DialogFooter className="p-6 bg-muted border-t flex flex-row items-center mt-auto shrink-0">
               <div className="flex justify-between w-full h-11 items-center">
                <div className="flex gap-3">
                  <Button variant="ghost" onClick={handleClose} disabled={isSubmitting} className="text-muted-foreground font-bold">
                    Cancel
                  </Button>
                  {currentTab > 0 && (
                    <Button variant="outline" onClick={handlePrevious} className="bg-card border-border font-bold px-5">
