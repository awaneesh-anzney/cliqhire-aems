"use client";
 ...
 export function HistoryContent({ clientId }: HistoryContentProps) {
   ...
   const getActionColor = (action: string) => {
     ...
     return "bg-muted-foreground shadow-black/10";
   };
   ...
   if (isLoading) {
     return (
       <div className="flex flex-col items-center justify-center p-24 space-y-4">
         ...
         <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] animate-pulse">Synchronizing Activity Data...</p>
       </div>
     );
   }
   ...
   if (error) {
     return (
       <div className="p-12 text-center bg-red-50/50 rounded-3xl border border-red-100 max-w-2xl mx-auto mt-8">
         ...
         <h3 className="text-lg font-black text-foreground mb-2 tracking-tight">Audit Log Retrieval Failed</h3>
         <p className="text-sm text-muted-foreground font-semibold mb-6">We encountered an error while fetching the activity history. Please try again.</p>
     ...
     <div className="bg-muted/30 rounded-3xl p-8 space-y-10 animate-in fade-in duration-500 min-h-[600px] flex flex-col">
       ...
       <div className="flex items-center justify-between px-2">
         <div className="space-y-1">
           ...
           <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest">Chronological timeline of all system actions</p>
         </div>
         ...
         <div className="flex items-center gap-2">
            <div className="flex items-center bg-card border border-border rounded-xl p-1 shadow-sm">
              <Button 
                variant="ghost" 
                size="icon" 
                className="h-8 w-8 rounded-lg text-muted-foreground hover:text-brand"
                disabled={page <= 1}
                onClick={() => setPage(p => Math.max(1, p - 1))}
              >
                <ChevronLeft className="w-4 h-4" />
              </Button>
              <span className="text-[10px] font-black px-3 text-muted-foreground uppercase">Page {page} of {pagination.totalPages}</span>
              <Button 
                variant="ghost" 
                size="icon" 
                className="h-8 w-8 rounded-lg text-muted-foreground hover:text-brand"
       ...
       <div className="flex-1 relative">
         {history.length === 0 ? (
           <div className="bg-card rounded-3xl border-2 border-dashed border-border p-20 text-center flex flex-col items-center">
             ...
             <h3 className="text-xl font-black text-foreground">Clean Slate</h3>
             ...
             {Object.entries(groupedHistory).map(([date, records]) => (
               <div key={date} className="relative">
                 ...
                 <div className="flex items-center gap-4 mb-8 sticky top-0 bg-transparent z-10 py-1">
                   ...
                   <h3 className="text-xs font-black text-foreground uppercase tracking-[0.15em]">{date}</h3>
                   ...
                   {records.map((record, index) => {
                     ...
                     return (
                       <div key={record._id || index} className="relative group animate-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${index * 50}ms` }}>
                         {/* Dot on Timeline */}
                         <div className={cn(
                           "absolute left-[-22px] top-4 w-5 h-5 rounded-full border-4 border-border flex items-center justify-center shadow-lg transition-transform group-hover:scale-125 z-20 text-white",
                           getActionColor(record.action)
                         )}>
                           {getActionIcon(record.action)}
                         </div>
 
                         <div className="bg-card rounded-2xl border border-border p-6 shadow-sm hover:shadow-xl hover:border-brand/20 transition-all duration-300">
                           <div className="flex items-start justify-between mb-4">
                             <div className="flex items-center gap-4">
                               <Avatar className="h-10 w-10 border-2 border-white ring-2 ring-border shadow-sm">
                                 <AvatarFallback className="bg-muted text-[10px] font-black text-muted-foreground uppercase tracking-tighter">{userInitials}</AvatarFallback>
                               </Avatar>
                               <div>
                                 <div className="flex items-center gap-2">
                                   <span className="text-sm font-black text-foreground leading-none">{userName}</span>
                                   <span className="text-[10px] font-black bg-muted text-muted-foreground px-2 py-0.5 rounded-full uppercase tracking-tighter">
                                     {record.entity_type}
                                   </span>
                                 </div>
                                 <p className="text-[11px] text-muted-foreground font-bold mt-1.5 flex items-center gap-1.5">
                                   <span className="text-brand lowercase font-black tracking-tight">{record.action}</span> 
                                   <span className="text-muted-foreground">•</span>
                                   <Clock className="w-3 h-3 text-muted-foreground" /> {formatDate(record.created_at)}
                                 </p>
                               </div>
                             </div>
                           </div>
                           ...
                           {record.changes?.after && Object.keys(record.changes.after).length > 0 && (
                             <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                               {Object.keys(record.changes.after).map((key) => {
                                 ...
                                 return (
                                   <div key={key} className="flex flex-col bg-muted/50 rounded-xl p-3 border border-border/50 hover:bg-card hover:shadow-md transition-all group/item">
                                     <span className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.1em] mb-2">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                                     <div className="flex items-center gap-2 overflow-hidden">
                                       {beforeVal !== undefined && (
                                         <>
                                           <span className="text-xs text-muted-foreground font-bold line-through opacity-50 truncate" title={formatValue(beforeVal)}>
                                             {formatValue(beforeVal)}
                                           </span>
                                           <ArrowRight className="w-3 h-3 text-muted-foreground shrink-0 group-hover/item:text-brand transition-colors" />
                                         </>
                                       )}
                                       <span className="text-xs text-foreground font-black truncate" title={formatValue(afterVal)}>
       ...
       {pagination.totalPages > 1 && (
         <div className="flex items-center justify-between pt-10 mt-6 border-t border-border">
           ...
           <div className="flex items-center gap-3">
             <Button
               variant="outline"
               size="sm"
               className="rounded-xl font-bold h-10 px-6 border-border hover:bg-muted disabled:opacity-30"
               onClick={() => {
