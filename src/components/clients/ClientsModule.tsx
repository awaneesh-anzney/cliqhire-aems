...
export default function ClientsModule({ moduleType = "clients" }: ClientsModuleProps) {
  ...
  const entityNamePlural = moduleType === 'leads' ? 'Leads' : 'Clients';
  ...
  const confirmDeleteSelected = async () => {
    ...
    try {
      ...
      await refetch();
      setSelectedRows(new Set());
      toast.success(`${selectedRows.size} ${entityName.toLowerCase()}(s) deleted successfully`);
    } catch (error) {
      toast.error(`Failed to delete selected ${entityNamePlural.toLowerCase()}. Please try again.`);
    } finally {
      setIsDeleting(false);
  ...
  const fetchStage = moduleType === 'clients' ? 'Signed' : (selectedClientStage === 'All' ? 'Lead,Engaged' : selectedClientStage);
  ...
  const handleConfirmChange = async () => {
    ...
    try {
      ...
      setError(error.message || `Failed to update ${entityName.toLowerCase()} stage. Please try again.`);
    }
  };
  ...
  if (!canViewClients) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <div className="text-center text-muted-foreground text-sm font-bold uppercase tracking-widest">You do not have permission to view {entityNamePlural.toLowerCase()}.</div>
      </div>
    );
  }
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <ConfirmDialog
        ...
        description={`Are you sure you want to update the ${entityName.toLowerCase()} stage?`}
        confirmText="Confirm"
        cancelText="Cancel"
        loading={isLoading}
        error={error}
        ...
        <div className="flex-shrink-0 bg-card rounded-[1.2rem] border border-border shadow-sm overflow-hidden flex flex-col">
         ...
         <div className="flex-shrink-0 bg-card rounded-[1.2rem] border border-border shadow-sm px-4 py-3 flex flex-col gap-2.5">
           <div className="flex flex-wrap items-center gap-3">
             ...
             {(searchInput || nameInput || clientIdInput || emailInput || phoneNumberInput || industryInput || locationInput || (selectedClientStage !== "All" && moduleType === 'leads')) && (
               <Button
                 variant="ghost"
                 size="sm"
           ...
           {advancedFiltersOpen && (
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 pt-3 border-t border-border/60 animate-in slide-in-from-top-2 duration-300">
               ...
               <div className="flex flex-col gap-1">
                 <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">{entityName} Name</label>
                 <input
                   type="text"
               ...
               {/* Client ID Filter */}
               <div className="flex flex-col gap-1">
                 <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">{entityName} ID</label>
                 <input
                   type="text"
            ...
            <Table className="w-full border-separate border-spacing-0 table-auto">
              ...
              <TableBody>
                {isLoading && allClients.length === 0 ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={10} className="h-64 text-center">
                       <p className="text-muted-foreground font-bold text-xs uppercase tracking-widest">No {entityNamePlural.toLowerCase()} available</p>
                    </TableCell>
                  </TableRow>
                ) : (
